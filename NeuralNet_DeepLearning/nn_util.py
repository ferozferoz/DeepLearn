...
def load_dataset():
    ...
    train_dataset = h5py.File('neuralnet_dataset/train_catvnoncat.h5', "r")
    test_dataset = h5py.File('neuralnet_dataset/test_catvnoncat.h5', "r")
...
def propagate(w,b,X,Y):
    ...
    dw = (1/m) * np.dot((A-Y),X.T)    # A = 1,m   m,n
    db = (1/m) * np.sum(A-Y)
...
def model(train_x, train_y, test_x, test_y, num_iterations, learning_rate):
    # initialize parameters with zeros (≈ 1 line of code)
    w, b = initialize_with_zero(train_x.shape[0])

    # Gradient descent (≈ 1 line of code)
    parameters, grads, costs = optimize(w, b, train_x, train_y, num_iterations, learning_rate)
...
train_x = train_x / 255
test_x = test_x / 255
d = model(train_x, train_y, test_x, test_y, 2000, 0.1)
